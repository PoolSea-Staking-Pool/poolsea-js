...
export default function runNetworkBalancesTests(web3: Web3, rp: RocketPool) {
	describe("Network Balances", () => {
		...
		it(printTitle("trusted nodes", "can submit network balances"), async () => {
			// Set parameters
			let block = 1;
			const totalBalance = web3.utils.toWei("10", "ether");
			const stakingBalance = web3.utils.toWei("9", "ether");
		...
		it(printTitle("trusted nodes", "cannot submit invalid network balances"), async () => {
			// Set parameters
			const block = 1;
			const totalBalance = web3.utils.toWei("9", "ether");
			const stakingBalance = web3.utils.toWei("10", "ether");
			const rethSupply = web3.utils.toWei("8", "ether");
