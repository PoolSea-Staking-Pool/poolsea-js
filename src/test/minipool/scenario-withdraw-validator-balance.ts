...
export async function withdrawValidatorBalance(
	...
	// Get parameters
	const [nodeFee] = await Promise.all([
	...
	const [balances1, minipoolBalances1] = await Promise.all([getBalances(), getMinipoolBalances()]);
	...
	const [balances2, minipoolBalances2] = await Promise.all([getBalances(), getMinipoolBalances(finalise)]);
	...
	const nodeBalanceChange = balances2.nodeWithdrawalEth
	...
	const depositPoolChange = balances2.depositPoolEth.sub(balances1.depositPoolEth);
	...
	if (_withdrawalBalance.gte(depositBalance)) {
		const depositType = await minipool.getDepositType();
		let userAmount = minipoolBalances1.userDepositBalance;
		const rewards = _withdrawalBalance.sub(depositBalance);
		const halfRewards = rewards.divn(2);
		const nodeCommissionFee = halfRewards.mul(web3.utils.toBN(nodeFee)).div(web3.utils.toBN(web3.utils.toWei("1")));
		...
		if (!(from === nodeWithdrawalAddress || from === nodeAddress)) {
			const refundBalance = await minipool.getNodeRefundBalance().then((value) => web3.utils.toBN(value));
