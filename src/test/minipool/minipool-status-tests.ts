...
export default function runMinipoolStatusTests(web3: Web3, rp: RocketPool) {
	describe("Minipool Status Tests", () => {
		...
		before(async () => {
			...
			// Create minipools
			stakingMinipool1 = (await createMinipool(web3, rp, {
				from: node,
				value: web3.utils.toWei("16", "ether"),
				gas: gasLimit,
			})) as MinipoolContract;
			stakingMinipool2 = (await createMinipool(web3, rp, {
				from: node,
				value: web3.utils.toWei("16", "ether"),
				gas: gasLimit,
			})) as MinipoolContract;
			stakingMinipool3 = (await createMinipool(web3, rp, {
				from: node,
				value: web3.utils.toWei("16", "ether"),
				gas: gasLimit,
			})) as MinipoolContract;
			...
			const stakingStatus1 = await stakingMinipool1.contract.methods
				.getStatus()
				.call()
				.then((value: any) => web3.utils.toBN(value));
			const stakingStatus2 = await stakingMinipool2.contract.methods
				.getStatus()
				.call()
				.then((value: any) => web3.utils.toBN(value));
			const stakingStatus3 = await stakingMinipool3.contract.methods
				.getStatus()
				.call()
				.then((value: any) => web3.utils.toBN(value));
		...
		it(printTitle("trusted nodes", "cannot submit a withdrawable event for a minipool while withdrawable submissions are disabled"), async () => {
			// Set parameters
			const startBalance = web3.utils.toWei("32", "ether");
			...
			await shouldRevert(
				submitWithdrawable(web3, rp, stakingMinipool1.address, {
					from: trustedNode1,
					gas: gasLimit,
				}),
		...
		it(printTitle("trusted nodes", "cannot submit a withdrawable event for an invalid minipool"), async () => {
			...
			await shouldRevert(
				submitWithdrawable(web3, rp, random, {
					from: trustedNode1,
					gas: gasLimit,
				}),
		...
		it(printTitle("random", "cannot execute status update without consensus"), async () => {
			...
			await shouldRevert(
				executeSetWithdrawable(web3, rp, stakingMinipool1.address, {
					from: random,
					gas: gasLimit,
				}),
