...
export async function rewardsClaimNode(web3: Web3, rp: RocketPool, options: SendOptions) {
	...
	const nodeWithdrawalAddress = await rp.node.getNodeWithdrawalAddress(options.from);
	...
	function getBalances() {
		...
		]).then(([claimIntervalTimeStart, nodeRpl]) => ({
	...
	const [details1, balances1] = await Promise.all([getDetails(), getBalances()]);
	...
	// Get updated balances
	const [details2, balances2] = await Promise.all([getDetails(), getBalances()]);

	// Calculate expected RPL claim amount
	const calcBase = web3.utils.toBN(web3.utils.toWei("1", "ether"));
	const claimPerc = calcBase.mul(details2.nodeRplStake).div(details2.totalRplStake);
	const expectedClaimAmount = details2.nodesRplShare.mul(claimPerc).div(calcBase);
