...
export async function placeBid(web3: Web3, rp: RocketPool, lotIndex: number, options: SendOptions) {
	...
	function getLotDetails(bidderAddress: string) {
		...
		]).then(([totalRplAmount, totalBidAmount, addressBidAmount, priceByTotalBids, currentPrice, claimedRplAmount, remainingRplAmount]) => ({
			totalRplAmount,
			totalBidAmount,
			addressBidAmount,
			priceByTotalBids,
	...
	function getLotPriceAtBlock() {
		return web3.eth.getBlock("latest").then((block: any) => rp.auction.getLotPriceAtBlock(lotIndex, block.number));
	}

	// Get initial lot details & balances
	const [lot1, balances1] = await Promise.all([getLotDetails(options.from), getBalances(options.from)]);
	...
	// Place bid
	const txReceipt = await rp.auction.placeBid(lotIndex, options);
	const txFee = gasPrice.mul(web3.utils.toBN(txReceipt.gasUsed));

	// Get updated lot details & balances
	const [lot2, balances2] = await Promise.all([getLotDetails(options.from), getBalances(options.from)]);
