...
export async function claimBid(web3: Web3, rp: RocketPool, lotIndex: number, options: SendOptions) {
	...
	function getContractDetails() {
		...
		]).then(([allottedRplBalance, remainingRplBalance]) => ({
			allottedRplBalance,
			remainingRplBalance,
		}));
	}
	...
	function getLotDetails(bidderAddress: string) {
		...
		]).then(([addressBidAmount, currentPrice]) => ({
			addressBidAmount,
			currentPrice,
		}));
	}
	...
	const [details1, lot1, balances1] = await Promise.all([getContractDetails(), getLotDetails(options.from), getBalances(options.from)]);
	...
	const [details2, lot2, balances2] = await Promise.all([getContractDetails(), getLotDetails(options.from), getBalances(options.from)]);
