...
export default function runAuctionTests(web3: Web3, rp: RocketPool) {
	describe("Auction Manager", () => {
		...
		it(printTitle("auction lot", "has correct price at block"), async () => {
			...
			const effectiveRPLStake = await rp.node.calculateTotalEffectiveRPLStake(0, 0, web3.utils.toWei("1", "ether"));
			...
			for (let vi = 0; vi < values.length; ++vi) {
				const v = values[vi];
				const price = await getLotPriceAtBlock(web3, rp, 0, v.block);
