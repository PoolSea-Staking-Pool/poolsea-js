...
export async function withdrawRpl(web3: Web3, rp: RocketPool, amount: string, options: SendOptions) {
	...
	function getStakingDetails(nodeAddress: string) {
		return Promise.all([
			...
			rocketNodeStaking.methods
				...
				.then((value: any) => web3.utils.toBN(value)),
			rocketNodeStaking.methods
				.getNodeRPLStake(nodeAddress)
				.call()
				.then((value: any) => web3.utils.toBN(value)),
			rocketNodeStaking.methods
				.getNodeEffectiveRPLStake(nodeAddress)
				.call()
				.then((value: any) => web3.utils.toBN(value)),
			rocketNodeStaking.methods
				.getNodeMinipoolLimit(nodeAddress)
				.call()
				.then((value: any) => web3.utils.toBN(value)),
	...
	// Withdraw RPL
	await rocketNodeStaking.methods.withdrawRPL(amount).send(options);
