	...
	describe("Contracts", () => {
		describe("Addresses", () => {
			...
			it(printTitle("User", "Can load multiple addresses"), async () => {
				const [rocketMinipoolQueueAddress, rocketMinipoolStatusAddress] = await rp.contracts.address(["poolseaMinipoolQueue", "poolseaMinipoolStatus"]);
		...
		describe("ABIs", () => {
			it(printTitle("User", "Can load single ABIs"), async () => {
				const minipoolManagerAbi = await rp.contracts.abi("poolseaMinipoolManager");
				assert.isArray(minipoolManagerAbi, "Loaded ABI is invalid");
			});

			it(printTitle("User", "Can load multiple ABIs"), async () => {
				const [rocketMinipoolQueueAbi, rocketMinipoolStatusAbi] = await rp.contracts.abi(["poolseaMinipoolQueue", "poolseaMinipoolStatus"]);
		...
		describe("Contracts", () => {
			it(printTitle("User", "Can load a single contract"), async () => {
				const rocketNetworkBalances = await rp.contracts.get("poolseaNetworkBalances");
			...
			it(printTitle("User", "Can load multiple contracts"), async () => {
				const [rocketNetworkFees, rocketNetworkPrices] = await rp.contracts.get(["poolseaNetworkFees", "poolseaNetworkPrices"]);
			...
			it(printTitle("User", "Can create a new contract instance"), async () => {
				const minipool = await rp.contracts.make("poolseaMinipoolDelegate", "0x1111111111111111111111111111111111111111");
