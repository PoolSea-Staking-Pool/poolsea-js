...
export async function setDAONodeTrustedBootstrapMember(web3: Web3, rp: RocketPool, _id: string, _url: string, _nodeAddress: string, options: SendOptions) {
	...
	function getTxData() {
		return Promise.all([rp.dao.node.trusted.node.getMemberID(_nodeAddress)]).then(([memberID]) => ({ memberID }));
	}
...
export async function setDAONodeTrustedBootstrapSetting(
	web3: Web3,
	rp: RocketPool,
	...
	if (typeof _value == "number" || typeof _value == "string")
		await assert(ds2.settingUintValue.eq(web3.utils.toBN(_value)), "DAO node trusted uint256 setting not updated in bootstrap mode");
...
export async function setDaoNodeTrustedBootstrapModeDisabled(web3: Web3, rp: RocketPool, options: SendOptions) {
	...
	// Get data about the tx
	function getTxData() {
		return Promise.all([rp.dao.node.trusted.node.getBootstrapModeDisabled()]).then(([bootstrapmodeDisabled]) => ({
...
export async function setDaoNodeTrustedBootstrapUpgrade(
	web3: Web3,
	rp: RocketPool,
	_type: string,
	_name: string,
	...
	function getContractAddressData(_contractAddress: string) {
		...
		]).then(([exists, name]) => ({ exists, name }));
	}

	// Get initial contract data
	const contract1 = await getContractData();
	...
	const contract2 = await getContractData();
	const [oldContractData, newContractData] = await Promise.all([getContractAddressData(contract1.address), getContractAddressData(contract2.address)]);

	// Initialise new contract from stored data
	const newContract = new web3.eth.Contract(decompressABI(contract2.abi), contract2.address);
	...
	if (_type == "upgradeABI" || _type == "addABI") {
		// Check ABI details
		const contractAbi = await rocketStorage.methods.getString(web3.utils.soliditySha3("contract.abi", _name)).call();
		const contract = new web3.eth.Contract(decompressABI(contractAbi), "0x0000000000000000000000000000000000000000");
